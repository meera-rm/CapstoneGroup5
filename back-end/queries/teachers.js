...
const deleteTeacher = async (id) => {
	try {
		return await db.one('DELETE FROM teachers WHERE id=$1 RETURNING *', id);
	} catch (error) {
		return error;
	}
};
...
const createTeacher = async (teachers) => {
	try {
		return await db.one(
			'INSERT INTO teachers (teacher_name, school_name, school_district, school_address,zipcode, state_name, class_subject) VALUES ($1, $2, $3, $4, $5, $6, $7 ) RETURNING *',
			[
...
const upDateTeacher = async (id, teachers) => {
	try {
		return await db.one(
			'UPDATE teachers SET teacher_name = $1, school_name = $2, school_district = $3, school_address = $4, zipcode = $5, state_name = $6, class_subject = $7 WHERE id = $8 RETURNING *',
			[
				teachers.teacher_name,
				teachers.school_name,
				teachers.school_district,
